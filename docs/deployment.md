...
3. `ALLOWED_ORIGINS` 환경 변수에 Vercel/VIP 도메인(예: `https://frontend-sigma-pearl-65.vercel.app,https://bodam.website,https://www.bodam.website`)을 설정한 뒤 `infra/k8s/backend/deployment.yaml`과 `infra/k8s/backend/service.yaml`을 적용합니다.
...
3. 환경 변수 `NEXT_PUBLIC_API_BASE_URL`과 `API_BASE_URL`을 API 엔드포인트(예: `https://api.bodam.website`)로 설정합니다.
