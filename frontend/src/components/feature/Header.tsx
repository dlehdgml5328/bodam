...
export default function Header() {
  ...
  const handleNotificationClick = (notification: any) => {
    ...
    } else if (notification.type === 'fire_incident') {
      // 화재 알림 클릭 시 기부 페이지로 이동
  ...
  useEffect(() => {
    const checkLoginStatus = async () => {
      ...
      // 로그인 상태이고 알림 권한이 없으면 요청
      if (loginStatus && permission === 'default') {
        // 3초 후에 자동으로 알림 권한 요청 (사용자 경험 개선)
        setTimeout(() => {
          requestPermission();
        }, 3000);
      }
  ...
  // 회원 기부내역 조회 (로그인 후)
  const handleMemberInquiry = async () => {
    ...
    try {
      const response = await apiRequest<any>(`/donations?donor_email=${encodeURIComponent(userEmail)}&limit=50`);
      const donations = response.donations || [];

      // API 응답을 모달에서 사용하는 형식으로 변환
      const formattedHistory = donations.map((d: any) => ({
        id: d.id,
        date: new Date(d.created_at),
        amount: Number(d.amount),
        fireStation: d.fire_station?.name || '알 수 없음',
        status: d.status,
        receiptIssued: d.needs_receipt && d.status === 'completed',
        message: d.message,
        donorName: userNickname || '기부자',
        verificationId: d.id,
        uniqueCode: d.toss_order_id || '',
        isGuest: false,
      }));

      setDonationHistory(formattedHistory);
      setInquiryEmail(userEmail);
      setShowDonationHistoryModal(true);
    } catch (error) {
      console.error('기부 내역 조회 실패:', error);
      alert('기부 내역을 불러오는데 실패했습니다.');
    }
  };
  ...
  const handleSignup = async () => {
    ...
    if (!/^01[0-9]{8,9}$/.test(phone)) {
      setSignupError('휴대폰 번호 형식이 올바르지 않습니다. (예: 01012345678)');
      return;
    }

    if (!idNumber.trim()) {
      setSignupError('주민등록번호를 입력해주세요.');
      return;
    }

    if (!/^\d{6}-?\d{7}$/.test(idNumber)) {
      setSignupError('주민등록번호 형식이 올바르지 않습니다. (예: 123456-1234567)');
      return;
    }
          ...
          <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md mx-4">
            ...
            <div className="mt-6 p-4 bg-blue-50 rounded-lg">
              <h4 className="text-sm font-medium text-blue-900 mb-3">
                <i className="ri-information-line mr-2"></i>회원가입 혜택
              </h4>
