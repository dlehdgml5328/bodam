...
export default function DonationsPage() {
  ...
  // 지역별 소방서 필터링 - DB에 저장된 region과 직접 매칭
  const getFireStationsByRegion = (region: string) => {
    console.log('🔍 지역 필터링:', region, '전체 소방서 수:', fireStations.length);
    const filtered = fireStations.filter((station) => station.region === region);
    console.log('✅ 필터링 결과:', filtered.length, '개 소방서');
    return filtered;
  };
